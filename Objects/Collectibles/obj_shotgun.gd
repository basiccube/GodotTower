...
func _on_CollectArea_body_entered(body):
	if body is obj_player:
		...
		if (obj_player.shotgunAnim == 0 && !obj_player.backupweapon && obj_player.state == global.states.handstandjump):
			...
			destroy()
		elif (obj_player.shotgunAnim == 1 && !obj_player.backupweapon && obj_player.state == global.states.handstandjump):

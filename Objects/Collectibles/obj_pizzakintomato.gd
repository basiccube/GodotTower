...
func _process(delta):
	...
	if (global.tomatofollow && $Sprite.animation != "intro"):
		...
		if (!global.panic):
			if (obj_player.velocity.x != 0):
				...
				$Sprite.animation = "idle"
		if (global.cheesefollow && utils.instance_exists_level("obj_pizzakincheese")):
	...
	if (global.panic && global.tomatofollow):
		$Sprite.animation = "panic"
	if (overlaps_body(obj_player)):
		if (obj_player.state != global.states.hurt && !global.tomatofollow):

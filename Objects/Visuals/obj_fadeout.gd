...
func _process(delta):
	...
	if (utils.instance_exists("obj_player")):
		for i in get_tree().get_nodes_in_group("obj_player"):
			if (fadein == 1 && (i.state == global.states.victory || i.state == global.states.door)):
				i.state = global.states.comingoutdoor
			elif (fadein == 1 && i.state == global.states.door && (i.sprite_index == "downpizzabox" || i.sprite_index == "uppizzabox")):

...
func _process(delta):
	...
	if (obj_player.state != global.states.gameover):
		...
		elif (obj_player.state == global.states.mach2 || obj_player.sprite_index == "dive" || obj_player.sprite_index == "machslideboost" || obj_player.state == global.states.climbwall || obj_player.state == global.states.handstandjump || obj_player.state == global.states.shoulderdash):
